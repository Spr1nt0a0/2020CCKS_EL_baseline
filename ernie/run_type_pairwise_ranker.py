...
@software: pycharm
@file: run_type_pairwise_ranker.py
@time: 2020/7/22 22:48
'''
...
def main(args):
    ...
    if args.do_train:
        ...
        if args.in_tokens:
            if args.batch_size < args.max_seq_len:
                raise ValueError('if in_tokens=True, batch_size should greater than max_sqelen, \
                        got batch_size:%d seqlen:%d' % (args.batch_size, args.max_seq_len))
    ...
    if args.do_train:
        ...
        while True:
            try:
                    ...
                    if args.is_classify:
                        log.info(
                            ...
                            (
                                ...
                                steps, outputs["loss"], outputs['acc'],
                                args.skip_steps / used_time,
                            ),
                        )
                        ce_info.append(
                            [outputs["loss"], used_time],
                        )
                ...
                save_path = os.path.join(
                    args.checkpoints, "step_" + str(steps),
                )
    ...
    if args.do_val:
        evaluate_wrapper(
            args, reader, exe, test_prog, test_pyreader,
    ...
    if args.do_test:
        predict_wrapper(
            args, reader, exe, test_prog, test_pyreader, graph_vars,
            current_epoch, steps,
        )
